...
13. Long-context reliability gate (regression block on relevance/stability/latency envelope)
14. Linux parity gate (runtime/voice/tools/observability API parity on Linux target)
15. Linux installer smoke gate (install/upgrade/channel rollback path on Linux target)
16. Distribution resilience report gate (aggregated parity + installer/rollback + runtime lifecycle blocking checks)
17. Distribution channel manifest readiness gate (WinGet/Homebrew/Flathub templates + placeholders)
18. API quickstart compatibility gate (OpenAI-compatible developer onboarding contract)
19. Release quality dashboard snapshot gate (final post-Linux benchmark/reliability artifact + trend deltas)
20. Mission success/recovery report pack export (public KPI artifact)
21. Disaster recovery gate (backup + verify + restore drill)
22. Compliance operations gate (access review + incidents + evidence export)
